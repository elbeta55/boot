...
async def buscar(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Realiza una búsqueda manual y envía los resultados al usuario que solicita la búsqueda."""
    ...
    bot = Bot(token=TOKEN)
    company = ' '.join(context.args[1:])  # Si deseas proporcionar una empresa específica en la búsqueda
    jobs = await fetch_jobs(query, company)
    if jobs:
        message = f"Resultados de búsqueda para '{query}':\n\n" + "\n\n".join(jobs)
        await send_long_message(bot, update.message.chat_id, message)
    else:
