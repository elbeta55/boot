...
async def notify_new_jobs() -> None:
    """Busca y notifica nuevos empleos cada hora exacta."""
    global last_results
...
async def wait_until_next_hour():
    """Calcula el tiempo restante hasta la siguiente hora exacta y espera hasta entonces."""
...
async def periodic_task():
    """Tarea periódica para buscar y notificar nuevos trabajos a cada hora exacta."""
    while True:
        await notify_new_jobs()
        await wait_until_next_hour()  # Espera hasta la próxima hora exacta

async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Envía un mensaje de bienvenida cuando se usa el comando /start."""
    await update.message.reply_text('¡Hola! Este bot busca automáticamente ofertas de trabajo para Spark Driver en Walmart Inc. y Spark Driver, realizando 30 búsquedas cada hora.')

async def time(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Envía la hora actual del servidor cuando se usa el comando /time."""
    now = datetime.datetime.now()
    time_message = f"La hora actual del servidor es: {now.strftime('%Y-%m-%d %H:%M:%S')}"
